...
.controller('ExpenseCtrl', function($scope, $http, $state, $ionicModal) {
    ...
    $scope.submitExpenseModal = function() {
        ...
        $http.post(
            ...
            .success(function() {
                ...
                $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded;charset=utf-8';
                $http.post(bestoonURL + '/q/expenses/', 'token=' + token)
                    .success(function(data) {
    ...
    $scope.delete = function(item) {
        $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded;charset=utf-8';
        $http.post(
                bestoonURL + '/edit/rmexpense/',
                'token=' + token + '&id=' + item.pk
            )
            .success(function(data) {
...
.controller('IncomeCtrl', function($scope, $http, $state, $ionicModal) {
        ...
        $scope.$on('$ionicView.enter', function(e) {
            ...
            $scope.delete = function(item) {
                $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded;charset=utf-8';
                $http.post(
                        bestoonURL + '/edit/rmincome/',
                        'token=' + token + '&id=' + item.pk
                    )
                    .success(function() {
                        // update the incomes part, containing the new one
                        $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded;charset=utf-8';
                        $http.post(bestoonURL + '/q/incomes/', 'token=' + token)
                            .success(function(data) {
                                $scope.incomes = JSON.parse(data);
                            })
                            .error(function() {
