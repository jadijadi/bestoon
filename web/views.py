    ...
    if 'requestcode' in request.POST:
        ...
        if not grecaptcha_verify(request):  # captcha was not correct
            context = {
                'message': 'کپچای گوگل درست وارد نشده بود. شاید ربات هستید؟'
                           ' کد یا کلیک یا تشخیص عکس زیر فرم را درست پر کنید. ببخشید که فرم به شکل اولیه برنگشته!'
        ...
        if User.objects.filter(email=request.POST['email']).exists():
            context = {
                'message': 'متاسفانه این ایمیل قبلا استفاده شده است. در صورتی که این ایمیل شما است،'
                           ' از صفحه ورود گزینه فراموشی پسورد رو انتخاب کنین. ببخشید که فرم ذخیره نشده. درست می شه'
            }  # TODO: forgot password
        ...
        if not User.objects.filter(username=request.POST['username']).exists():
            ...
            body = " برای فعال کردن اکانت بستون خود روی لینک روبرو کلیک کنید:" \
                   " <a href=\"{}?code={}\">لینک رو به رو</a> ".format(
                                                                request.build_absolute_uri('/accounts/register/'), code)
            ...
            context = {
                'message': 'متاسفانه این نام کاربری قبلا استفاده شده است.'
                           ' از نام کاربری دیگری استفاده کنید. ببخشید که فرم ذخیره نشده. درست می شه'
            }  # TODO: forgot password
            # TODO: keep the form data
            return render(request, 'register.html', context)
        ...
        if Passwordresetcodes.objects.filter(
            ...
            context = {
                'message': 'اکانت شما ساخته شد. توکن شما {} است.'
                           ' آن را ذخیره کنید چون دیگر نمایش داده نخواهد شد! جدی!'.format(this_token)
            }
            return render(request, 'index.html', context)
        else:
            context = {
...
def whoami(request):
    if 'token' in request.POST:
        ...
        return JsonResponse({'user': this_user.username}, encoder=JSONEncoder)  # return {'user':'USERNAME'}

    else:
        return JsonResponse({'message': 'لطفا token را نیز ارسال کنید .'}, encoder=JSONEncoder)  #
...
def submit_expense(request):
    ...
    return JsonResponse({'status': 'ok'}, encoder=JSONEncoder)  # return {'status':'ok'}
